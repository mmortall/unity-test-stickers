...
using System.IO;
using System;
using UnityEditor.iOS.Xcode.New.PBX;

namespace UnityEditor.iOS.Xcode.New.Extensions
{
    ...
    */
    public static class PBXProjectExtensions
    {
		...
		/// prefixed with the parent app bundle ID.</param>
		/// <param name="infoPlistPath">Path to the app extension Info.plist document.</param>
		public static string AddAppExtensionStickers(this PBXProject proj, string mainTargetGuid, 
			string name, string bundleId, string infoPlistPath)
		{
			string ext = ".appex";
			var newTargetGuid = proj.AddTarget(name, ext, "com.apple.product-type.app-extension.messages-sticker-pack");
			...
			foreach (var configName in proj.BuildConfigNames())
			{
				...
				proj.SetBuildPropertyForConfig(configGuid, "INFOPLIST_FILE", infoPlistPath);
				proj.SetBuildPropertyForConfig(configGuid, "PRODUCT_BUNDLE_IDENTIFIER", bundleId);
				proj.SetBuildPropertyForConfig(configGuid, "ASSETCATALOG_COMPILER_APPICON_NAME", "iMessage App Icon");
			}
...
} // namespace UnityEditor.iOS.Xcode.New
